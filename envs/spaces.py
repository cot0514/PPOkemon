...
N_MOVES: int = 4
# power_norm(1) + is_special(1) + pp_frac(1) = 3
...
def _encode_move(move: Move | None) -> np.ndarray:
    """3-dim 기술 인코딩: power_norm(1) + is_special(1) + pp_frac(1).
    ...
    pp_frac = move.current_pp / move.max_pp if move.max_pp > 0 else 1.0

    return np.array([power, is_special, pp_frac], dtype=np.float32)
