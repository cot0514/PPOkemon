...
from pokemon_pool.pokemon_list import N_POKEMON_POOL, POKEMON_TO_IDX
...
def _encode_active_slot(pokemon: Pokemon) -> np.ndarray:
    parts: list[np.ndarray] = [
        ...
        np.array(
            [pokemon.boosts.get(k, 0) / MAX_BOOST for k in STAT_KEYS], dtype=np.float32
        ),
    ]
    encoded = np.concatenate(parts)
    assert encoded.shape == (DIM_ACTIVE_SLOT,), (
        f"Expected {DIM_ACTIVE_SLOT}, got {encoded.shape}"
    )
    return encoded
...
def _encode_bench_slot(pokemon: Pokemon | None) -> np.ndarray:
    ...
    assert encoded.shape == (DIM_BENCH_SLOT,), (
        f"Expected {DIM_BENCH_SLOT}, got {encoded.shape}"
    )
    return encoded
